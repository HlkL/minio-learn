...
uploadFile = async () => {
    ...
    if (fileSize <= chunkSize) {
        ...
        $.ajax({
            async: false,
            url: "http://127.0.0.1:8080/file", type: 'POST', contentType: false, processData: false, data: formData,
        ...
        await $.ajax({
            url: "http://127.0.0.1:8080/file/create",
            type: 'POST',
            ...
            success: async function (res) {
                ...
                if (res.code === 200) {
                    ...
                    composeFile(res.uploadId, file.name, chunkCount, fileSize, file.contentType)
                    return;
                }
                if (res.code === 0) {
                    console.log("当前文件上传情况：秒传")
                    videoPlay(res.filePath, res.suffix)
                    return
                }
...
composeFile = (uploadId, fileName, chunkSize, fileSize, contentType) => {
    ...
    $.ajax({
        url: "http://127.0.0.1:8080/file/complete",
        type: 'POST',
        contentType: "application/json",
        processData: false,
        data: composeParams,
